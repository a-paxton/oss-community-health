...
# get unique comment authors per project
comment_projectwise_authors = comments_frame %>% ungroup() %>%
  dplyr::select(project, author_name) %>%
  dplyr::group_by(project) %>%
  distinct() 

# get unique ticket authors per project
ticket_projectwise_authors = tickets_frame %>% ungroup() %>%
  dplyr::select(project, author_name) %>%
  dplyr::group_by(project) %>%
  distinct() 

# get number of total unique authors per project
total_projectwise_authors = rbind.data.frame(comment_projectwise_authors,
                                             ticket_projectwise_authors) %>%
  group_by(project) %>%
  distinct() %>%
  summarize(unique_authors = n())
...
pander(total_projectwise_authors, style = "rmarkdown")

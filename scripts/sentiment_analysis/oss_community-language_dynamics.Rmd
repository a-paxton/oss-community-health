...
tickets_frame = tickets_frame %>% ungroup() %>%
  ...
  # get time in seconds and read creation date properly
...
# add contribution type to ticket dataframe
tickets_frame = tickets_frame %>%
  mutate(type_family = 'post')

# add contribution type to ticket dataframe and filter out comments
# associated with tickets that have been filtered out due to date modified
...
creators_v_commenters_emotion_by_project_time = lmer(compound_emotion ~ 0 + project * type * author_group * ns(date) +
                                                       (1 | author_name),
                                                     data = sentiment_frame,
                                                     REML=TRUE)
...
```{r plot-model-emotion-by-contribution-and-membership, include=FALSE, echo=FALSE, eval=TRUE}
...
```{r plot-model-emotion-by-contribution-and-membership-and-time, include=FALSE, echo=FALSE, eval=TRUE}
...
# membership plot
sentiment_contribution_member_time_plot = ggplot(data=member_frame,
                                                 aes(x=date,
                                                     y=compound_emotion,
                                                     colour=type)) +
...
# nonmembership plot
sentiment_contribution_nonmember_time_plot = ggplot(data=nonmember_frame,
                                                    aes(x=date,
                                                        y=compound_emotion,
                                                        colour=type)) +
  #stat_smooth() +
...
# save full version
ggsave(filename = '../../figures/sentiment_analysis/ossc-sentiment_membership_contribution_time-by_project.jpg',
       units = "in", height = 8, width = 12,
       grid.arrange(
         top=grid::textGrob("Message sentiment by contribution\nand community membership",
                      gp=grid::gpar(fontsize=14)),
         sentiment_contribution_member_time_plot,
         sentiment_contribution_nonmember_time_plot,
...
ggsave(filename = '../../figures/sentiment_analysis/ossc-sentiment_membership_contribution_time-by_project-knitr.jpg',
